...
public class EXList<T> extends AbstractEXList<T> implements ListItemRenderer<T> {
	...
	public void addItem(final ListItem<T> item) {
		addChild(item);
		// addChild(new EXContainer(getChildren().size() + "",
		// "li").setStyle("list-style","none").addChild(item));
		item.addEvent(new Event() {
			...
			@Override
			public boolean ServerAction(final Container container, Map<String, String> request) throws UIException {
			...
			public void ClientAction(JQuery container) {
				container.IF(container.clone().getAttribute("lstdisabled").equal("true"), container.clone(),
						container.clone().mask().server(this));
